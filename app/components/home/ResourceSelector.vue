...
<template>
  <section aria-labelledby="choose-dataset" class="border-t border-[#e5e5e5] pt-8">
    <div class="mb-6 max-w-2xl">
      <h2 id="choose-dataset" class="text-2xl font-bold leading-8 text-[#161616]">
    ...
    <div class="grid gap-2">
      ...
      >
        <span
          class="flex h-7 w-7 shrink-0 items-center justify-center rounded-md border text-[13px]"
          :class="selectedId === resource.id
            ? 'border-[#000091] bg-[#000091] text-white'
            : 'border-[#e5e5e5] bg-[#f6f6f6] text-[#555555]'"
          aria-hidden="true"
        >
        ...
        <span class="min-w-0 flex-1">
          ...
          <span class="block truncate text-[11px] leading-4 text-[#555555]">
            {{ resource.organization }}
          </span>
        </span>
        <span class="shrink-0 text-[11px] text-[#555555]">
          {{ selectedId === resource.id ? "Sélectionné" : "Choisir" }}
        </span>
      </button>
    </div>

    <div class="mt-5 flex justify-end border-t border-[#e5e5e5] pt-5">
      <NuxtLink
        v-if="selectedResource"
        class="agent-focusable agent-pressable inline-flex h-9 items-center gap-2 rounded-md bg-[#000091] px-3 text-[12px] font-medium text-white hover:bg-[#1212ff]"
