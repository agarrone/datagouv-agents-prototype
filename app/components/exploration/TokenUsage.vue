...
<template>
  <details ref="details" class="relative h-6 text-[12px] leading-[1.4] text-[#5d5d5d]">
    <summary
      class="agent-focusable flex h-6 w-6 cursor-pointer list-none items-center justify-center rounded hover:bg-[#eee] [&::-webkit-details-marker]:hidden"
    ...
    >
      <i
        ...
        :class="total ? 'text-[#5d5d5d]' : 'text-[#929292]'"
      />
    </summary>
    <div class="absolute bottom-7 left-0 z-30 w-[220px] rounded border border-[#e5e5e5] bg-white p-2 shadow-[0_2px_4px_rgba(0,0,0,0.04),2px_4px_16px_rgba(0,0,0,0.12)]">
      <template v-if="total">
        <p class="mb-1 text-[11px] font-medium leading-4 text-[#161616]">Nombre de tokens utilisés</p>
        <div v-if="typeof usage?.inputTokens === 'number'" class="mt-1 flex items-center justify-between gap-4 text-[11px] leading-4 text-[#5d5d5d]">
          <span>Entrée</span>
          <span class="tabular-nums">{{ usage.inputTokens.toLocaleString("fr-FR") }}</span>
        </div>
        <div v-if="typeof usage?.outputTokens === 'number'" class="mt-1 flex items-center justify-between gap-4 text-[11px] leading-4 text-[#5d5d5d]">
          <span>Réponse</span>
      ...
      <p v-else class="text-[11px] leading-4 text-[#5d5d5d]">Nombre de tokens utilisés indisponible</p>
    </div>
  </details>
</template>
