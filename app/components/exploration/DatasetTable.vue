    ...
    <table class="w-full min-w-[58rem] border-collapse text-sm">
      <thead>
        <tr class="bg-[#f6f6f6] text-left">
          <th v-for="column in columns" :key="column" class="border-b border-r border-[#ddd] px-4 py-3 font-bold">
            {{ column }}
            <span v-if="showTypes" class="mt-1 block font-normal text-[#666]">{{ typeFor(column) }}</span>
          </th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(row, rowIndex) in rows" :key="rowIndex">
          <td v-for="column in columns" :key="column" class="max-w-64 truncate border-b border-r border-[#ddd] px-4 py-3">
            {{ row[column] }}
          </td>
        </tr>
      </tbody>
