...
<template>
  <main class="min-h-dvh bg-white text-[#161616]">
    <div class="mx-auto w-full max-w-[960px] px-4 py-8 sm:px-6 lg:py-12">
      <NuxtLink class="text-[13px] font-medium text-[#000091] underline underline-offset-4 hover:text-[#1212ff]" to="/documentation">
        Retour à la documentation
      </NuxtLink>

      <header class="mt-10 pb-10">
        <p class="text-[13px] font-medium text-[#666]">Documentation technique du prototype</p>
        ...
        <p class="mt-5 max-w-3xl text-[17px] leading-7 text-[#3a3a3a]">Cette page décrit l’implémentation actuelle : orchestration AI SDK, outils, données échangées, visualisations, feedback et garde-fous. Elle documente un prototype, pas une architecture cible définitive.</p>
      </header>

      <nav aria-label="Sommaire" class="border border-[#ddd] bg-[#f6f6f6] p-5">
        <p class="text-[11px] font-medium uppercase tracking-[0.05em] text-[#666]">Sommaire</p>
        <ol class="mt-3 grid gap-2 text-[13px] sm:grid-cols-2">
          <li v-for="link in summaryLinks" :key="link[0]"><a class="text-[#000091] underline underline-offset-2" :href="`#${link[0]}`">{{ link[1] }}</a></li>
        </ol>
      </nav>
      ...
      <DocumentationSection id="model" eyebrow="Génération" title="Fournisseur et modèle" description="Le choix du fournisseur est réalisé côté serveur, sans exposer de clé au navigateur.">
        <dl class="grid gap-px overflow-hidden border border-[#ddd] bg-[#ddd] sm:grid-cols-2">
          <div class="bg-white p-4"><dt class="text-[11px] uppercase text-[#666]">Priorité 1</dt><dd class="mt-1 text-[13px]">Vercel AI Gateway avec clé explicite</dd></div>
          <div class="bg-white p-4"><dt class="text-[11px] uppercase text-[#666]">Priorité 2</dt><dd class="mt-1 text-[13px]">Endpoint OpenAI-compatible, dont Albert</dd></div>
          <div class="bg-white p-4"><dt class="text-[11px] uppercase text-[#666]">Priorité 3</dt><dd class="mt-1 text-[13px]">Jeton OIDC fourni par Vercel</dd></div>
          <div class="bg-white p-4"><dt class="text-[11px] uppercase text-[#666]">Modèle Gateway par défaut</dt><dd class="mt-1 font-mono text-[13px]">openai/gpt-5.4-mini</dd></div>
        </dl>
      ...
      <DocumentationSection id="tools" eyebrow="Capacités" title="Outils disponibles" description="Le modèle choisit un tool, mais son contrat Zod et son exécution restent sous le contrôle de l’application.">
        <div class="grid gap-3">
          <article v-for="tool in tools" :key="tool.name" class="border border-[#ddd] p-4">
            <div class="flex flex-wrap items-center justify-between gap-2">
              <h3 class="font-mono text-[13px] font-semibold text-[#000091]">{{ tool.name }}</h3>
              <span class="bg-[#eee] px-2 py-0.5 text-[10px] text-[#555]">{{ tool.runs }}</span>
            </div>
      ...
      <DocumentationSection id="prompts" eyebrow="Instructions" title="Organisation des prompts" description="Les instructions sont découpées par responsabilité afin de rester lisibles, testables et portables.">
        <div class="grid gap-2">
          <article v-for="layer in promptLayers" :key="layer[0]" class="border border-[#ddd] px-4 py-3">
